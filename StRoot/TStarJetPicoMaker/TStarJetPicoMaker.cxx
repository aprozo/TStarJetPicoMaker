...
ClassImp(TStarJetPicoMaker)
   ...
   TStarJetPicoMaker::TStarJetPicoMaker(std::string outFileName, TChain *mcTree, inputMode input, std::string name,
                                        int nFiles, int trigSet)
   : StMaker(name.c_str()),
     mEventSelector(new TStarJetEventCuts),
     mV0Selector(new TStarJetV0Cuts),
     ...
     mBemcTables(nullptr),
     mBemcMatchedTracks(),
     mRefMultCorr(nullptr),
     mGRefMultCorr(nullptr),
     mCallsToMake(0),
     mNEvents(0),
     mNAcceptedEvents(0),
     mMakeV0(kFALSE),
...
Int_t TStarJetPicoMaker::InitInput()
{
   ...
   case InputMuDst:
      if (!mMuDstMaker) {
         LOG_ERROR << "TStarJetPicoMaker: MuDst specified as input, but no MuDstMaker in chain" << endm;
         return kStStop;
      }
   ...
   case InputPicoDst:
      if (!mPicoDstMaker) {
         LOG_ERROR << "TStarJetPicoMaker: PicoDst specified as input, but no PicoDstMaker in chain" << endm;
         return kStStop;
      }
...
void TStarJetPicoMaker::InitMakers()
{
   ...
   case FillGRefAndRefMultCorr:
      ...
      break;
   case FillGRefMultCorr: mGRefMultCorr = CentralityMaker::instance()->getgRefMultCorr(); break;
   case FillGRefMultCorrP16ID:
      ...
      break;
   case FillGRefMultCorrVPDMB30: mGRefMultCorr = CentralityMaker::instance()->getgRefMultCorr_VpdMB30(); break;
   case FillGRefMultCorrVPDNoVtx: mGRefMultCorr = CentralityMaker::instance()->getgRefMultCorr_VpdMBnoVtx(); break;
   case FillRefMultCorr: mRefMultCorr = CentralityMaker::instance()->getRefMultCorr(); break;
   default:
...
Int_t TStarJetPicoMaker::MakeMuDst()
{
   ...
   // LOG_INFO << "MUDST EVENT ID = " << (unsigned) mMuDst->event()->eventId() << " AND RUN ID = " << (unsigned)
   // mMuDst->event()->runId() << endm;
   ...
   switch (SelectVertex()) {
   case kStErr: return kStOk; break;
   case kStFatal: return kStFatal; break;
   default: break;
   }
 ...
 */
    ...
    */
   if (!mEventSelector->AcceptEvent(mMuInputEvent))
      return kStOk;
...
Int_t TStarJetPicoMaker::MakePicoDst()
{
    ...
    */
   if (!mEventSelector->AcceptEvent(mPicoInputEvent))
      return kStOk;
...
Int_t TStarJetPicoMaker::SelectVertex()
{
   ...
   if (mMakeMC && mStMiniMcEvent->nUncorrectedPrimaries() ==
                     0) { // since primaries mean there is a vertex //!mStMiniMcEvent->primaryVertex()) {
      LOG_DEBUG << "TStarJetPicoMaker: No StMcVertex for this event" << endm;
      return kStErr;
   }

   switch (mVertexMode) {
   case TStarJetPicoMaker::VpdOrRank: MatchToVpd(); break;
   case TStarJetPicoMaker::Vpd:
      ...
      break;
   case TStarJetPicoMaker::Rank: mMuDst->setVertexIndex(0); break;
   default: LOG_ERROR << "TStarJetPicoMaker: Unrecognized vertex selector option - exiting" << endm; return kStFatal;
   }
   return kStOk;
}
...
Bool_t TStarJetPicoMaker::MatchToVpd()
{
   ...
   if (tofheader) {
      ...
      for (Int_t i = 0; i < nVertices; ++i) {
         ...
         // I.M. 2022, not using upstream cut now: if (fabs(vz-vpdVz) < mdVzMax) {
         usedVertex = i;
         break;
         //}
      }
...
Bool_t TStarJetPicoMaker::MuProcessPrimaryTracks()
{
   ...
   for (UInt_t i = 0; i < nTracks; ++i) {
      ...
      if (/*muTrack->momentum().z() != muTrack->momentum().z()*/ isnan(muTrack->momentum().z())) {
         // LOG_INFO << "UNDEFINED MU PZ (primary track " << (unsigned) i << ") in event " << (unsigned)
...
Float_t TStarJetPicoMaker::ComputeXY(const StThreeVectorF &pos, const StPhysicalHelixD &helix)
{
   ...
   Double_t dPosCenter =
      TMath::Sqrt((pos.x() - xCenter) * (pos.x() - xCenter) + (pos.y() - yCenter) * (pos.y() - yCenter));
...
Bool_t TStarJetPicoMaker::MuProcessBEMC()
{
   ...
   if (!mEmcCollection || !mMuEmcCollection)
      return kFALSE;
   ...
   for (UInt_t i = 1; i <= mBEMCModules; ++i) {
      ...
      for (UInt_t j = 0; j < mEmcTowerHits.size(); ++j) {
         ...
         if (abs(tow->module()) > mBEMCModules || abs(tow->eta()) > mBEMCTowPerModule || tow->sub() > mBEMCSubSections)
            continue;
         ...
         if (mTowerStatusMode == RejectBadTowerStatus && towerStatus != 1)
            continue;
         ...
         for (unsigned k = 0; k < mBemcMatchedTracks.size(); ++k) {
            ...
            TStarJetPicoPrimaryTrack *matchedTrack =
               (TStarJetPicoPrimaryTrack *)mEvent->GetPrimaryTracks()->At(match.trackId);
            matchedTrack->SetEtaDiffHitProjected(dEta);
            matchedTrack->SetPhiDiffHitProjected(dPhi);
         }

         if (jetTower.GetNAssocTracks() > 0)
            nMatchedTowers++;
...
void TStarJetPicoMaker::MuProcessTriggerObjects()
{
   ...
   for (int i = 0; i < 4; ++i)
      mEvent->GetHeader()->SetHighTowerThreshold(i, mTriggerSimu->bemc->barrelHighTowerTh(i));
   for (int i = 0; i < 3; ++i)
   ...
   // LOG_INFO << "wisdom: High Tower trigger thresholds: bht0 " << bht0 << " bht1: " << bht1 << " bht2: " << bht2 << "
   // bht3: " << bht3 << endm;
   /*LOG_DEBUG*/
...
Bool_t TStarJetPicoMaker::MuFillHeader()
{
   ...
   if (mRefMultCorr) {
      ...
      mRefMultCorr->initEvent(mMuInputEvent->refMult(), mMuInputEvent->primaryVertexPosition().z(),
                              mMuInputEvent->runInfo().zdcCoincidenceRate());
   ...
   if (mGRefMultCorr) {
      ...
      mGRefMultCorr->initEvent(mMuInputEvent->grefmult(0), mMuInputEvent->primaryVertexPosition().z(),
                               mMuInputEvent->runInfo().zdcCoincidenceRate());
      mEvent->GetHeader()->SetGRefMultCorrRank0(mGRefMultCorr->getRefMultCorr());

      mGRefMultCorr->init(mMuInputEvent->runId());
      mGRefMultCorr->initEvent(mMuInputEvent->grefmult(), mMuInputEvent->primaryVertexPosition().z(),
                               mMuInputEvent->runInfo().zdcCoincidenceRate());
   ...
   if (mMuDst->btofHeader())
      mEvent->GetHeader()->SetvpdVz(mMuDst->btofHeader()->vpdVz());
   else
      mEvent->GetHeader()->SetvpdVz(9999);
...
void TStarJetPicoMaker::MuProcessMCEvent()
{
   ...
   while (StTinyMcTrack *track = (StTinyMcTrack *)next()) {
      ...
      if (!track->isPrimary()) {
         continue; // should effectively remove this non-primary from the event by not adding it to mMCEvent at the end
                   // of this for loop.
      }
      ...
      // if it has a valid GEANT-3 code
      // (https://www.star.bnl.gov/public/comp/simu/newsite/gstar/Manual/particle_id.html), convert this code to PDG ID
      // and save the particle info in mTrack
      if (track->geantId() > 0 && track->geantId() < 51) { // track->particleDefinition()){
         ...
         mTrack.SetdEdx(pdg_id); // note to self: SetdEdx doesn't actually calculate dE/dx for this particle, it just
                                 // holds the PDG id without modification.
      } else {
      ...
      //  nCount++; //this was in the original version of the maker, but its purpose is to hand the SetNOfPrimaryTracks
      //  call the number of primaries, while the AddPrimaryTrack function already does that (see
      //  TStarJetPicoEvent::AddPrimaryTrack for proof).
   ...
   mMCEvent->GetHeader()->SetPrimaryVertexX(
      mStMiniMcEvent
         ->mcVertexX()); // this should be mcVertexX rather than VertexX, right? //primaryVertex()->position().x());
   ...
   for (int i = 0; i < mMuDst->v0s()->GetEntries(); ++i) {
      ...
      if (mV0Selector->AcceptV0(mMuDst, v0MuDst)) {
         ...
         for (int j = 0; j < mMuDst->globalTracks()->GetEntries(); ++j) {
            ...
            if (negGlobalFound && posGlobalFound)
               break;
         }
         mEvent->AddV0(&jetV0);
      }
...
bool TStarJetPicoMaker::LoadEvent()
{
   ...
   if (mStMiniMcEvent->eventId() == eventID && mStMiniMcEvent->runId() == runID)
      return true;
